...
//                 // Sort the ride requests by distance from the driver
//                 if (driverLocation) {
...
//                     }).sort((a, b) => a.distance - b.distance); // Sorting in ascending order of distance

//                     setRideRequests(sortedRequests);
//                 }
//             }
//         );
...
//                 numRides: increment(1), // Increment the numRides field by 1
//             });
...
import DriverHeroSection from './DriverHeroSection';
import { collection, onSnapshot, updateDoc, doc, increment, getDoc } from 'firebase/firestore';
import { db } from '../firebaseConfig';
import { useAuth } from '../context/AuthContext';
...
export default function Driver() {
    ...
    const handleRequest = async (requestId, status) => {
        try {
            ...
            if (requestSnapshot.exists()) {
                ...
                // If status is 'accepted', update the request and set the driver's ID
                if (status === "accepted") {
                    await updateDoc(requestDoc, {
                        status: "accepted",
                        driverId: user.uid,  // Set the driver's ID
                    });

                    // Update driver's number of rides in Firestore
    ...
    return (
        <div className="relative bg-gray-50 min-h-screen">
            {/* Driver Navigation */}
            <DriverNav toggleMenu={toggleMenu} isMenuOpen={isMenuOpen} />
            ...
            <div className="p-6 space-y-8">
                ...
                <div className="bg-white shadow rounded-lg p-6">
                    <h1 className="text-2xl font-bold text-gray-800 mb-4">Ride Requests</h1>
